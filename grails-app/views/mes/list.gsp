...
<html>
    ...
    <!-- botones -->
        <div class="btn-toolbar toolbar" style="margin-bottom: 15px">
            ...
            <div class="btn-group">
                <g:link action="form" class="btn btn-default btnCrear">
                    <i class="fa fa-clipboard-list"></i> Nuevo Mes
                </g:link>
            </div>
        </div>
        ...
        <table class="table table-condensed table-bordered table-striped">
            ...
            <tbody>
                <g:each in="${mesInstanceList}" status="i" var="dataInstance">
                    <tr data-id="${dataInstance.id}">
                        <td>${fieldValue(bean: dataInstance, field: "numero")}</td>
        ...
        <script type="text/javascript">
            ...
            function submitForm() {
                ...
                $btn.replaceWith(spinner);
                    $.ajax({
                        type    : "POST",
                        url     : '${createLink(controller: 'Mes', action:'save_ajax')}',
                        data    : $form.serialize(),
            ...
            function deleteRow(itemId) {
                bootbox.dialog({
                    ...
                    message : "<i class='fa fa-trash fa-2x pull-left text-danger text-shadow'></i><p>" +
                        "¿Está seguro que desea eliminar el Mes? Esta acción no se puede deshacer.</p>",
                    buttons : {
                        cancelar : {
                            label     : "Cancelar",
