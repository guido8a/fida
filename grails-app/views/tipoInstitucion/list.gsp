...
<html>
    <head>
        ...
        <title>Tipos de Institución</title>
    </head>
    <body>
    ...
    <!-- botones -->
        ...
        <table class="table table-condensed table-bordered table-striped">
            ...
            <tbody>
                <g:each in="${tipoInstitucionInstanceList}" status="i" var="dataInstance">
        ...
        <script type="text/javascript">
            ...
            function submitForm() {
                ...
                $btn.replaceWith(spinner);
                    $.ajax({
                        type    : "POST",
                        url     : '${createLink(controller: 'tipoInstitucion', action:'save_ajax')}',
                        data    : $form.serialize(),
            ...
            function deleteRow(itemId) {
                bootbox.dialog({
                    ...
                    message : "<i class='fa fa-trash fa-2x pull-left text-danger text-shadow'></i><p>" +
                        "¿Está seguro que desea eliminar el Tipo seleccionado? Esta acción no se puede deshacer.</p>",
                    buttons : {
                        cancelar : {
                            label     : "Cancelar",
            ...
            function createEditRow(id) {
                ...
                $.ajax({
                    type    : "POST",
                    url     : "${createLink(controller: 'tipoInstitucion', action:'form_ajax')}",
                    data    : data,
                    success : function (msg) {
                        var b = bootbox.dialog({
                            id      : "dlgCreateEdit",
                            title   : title + " Tipo de Institución",
                            message : msg,
                            buttons : {
                                cancelar : {
                                    label     : "Cancelar",
