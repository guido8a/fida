...
<div class="btn-toolbar toolbar">
    ...
    <div class="btn-group">
        <a href="#" class="btn btn-primary btnCrear">
            <i class="fa fa-file"></i> Nuevo Tipo de Garantía
        </a>
...
<div style="margin-top: 30px; min-height: 400px" class="vertical-container">
    <p class="css-vertical-text">Tipo de Garantía</p>
...
<script type="text/javascript">
    ...
    function cargarTablaTipoGarantia (){
        $.ajax({
            type: 'POST',
            url: '${createLink(controller: 'tipoGarantia', action: 'tablaTipoGarantia')}',
            data:{
            },
            success: function (msg) {
                $("#tablaTipoGarantia").html(msg)
            }
        });
    }
    ...
    function createEditTipoGarantia(id) {
        ...
        $.ajax({
            type    : "POST",
            url     : "${createLink(controller: 'tipoGarantia', action:'form_ajax')}",
            data    : data,
            success : function (msg) {
                var b = bootbox.dialog({
                    id    : "dlgCreateEditTipoGarantia",
                    title : title + " Tipo de Garantía",
                    message : msg,
                    buttons : {
                        cancelar : {
                            label     : "Cancelar",
        ...
        var borrarTipoGarantia = {
            ...
            action           : function () {
                bootbox.confirm({
                    ...
                    message: "Está seguro de borrar este Tipo de Garantía? Esta acción no puede deshacerse.",
                    buttons: {
                        cancel: {
                    ...
                    callback: function (result) {
                        if(result){
                            ...
                            $.ajax({
                                type: 'POST',
                                url: '${createLink(controller: 'tipoGarantia', action: 'borrarTipoGarantia_ajax')}',
                                data:{
                                    id: id
                                },
                                success: function (msg) {
                                    dialog.modal('hide');
                                    if(msg == 'ok'){
                                        log("Tipo de garantía borrada correctamente","success");
                                        setTimeout(function () {
                                            cargarTablaTipoGarantia();
                                        }, 1000);
                                    }else{
                                        log("Error al borrar el Tipo de garantía", "error")
                                    }
                                }
                            });
                        }
