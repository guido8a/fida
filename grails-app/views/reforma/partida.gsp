...
<%@ page import="modificaciones.DetalleReforma; parametros.proyectos.Fuente; seguridad.Persona; parametros.Anio" contentType="text/html;charset=UTF-8" %>
<html>
    <head>
...
//                        dataDestino.monto = $("#monto").val();
                ...
                <g:set var="detRef" value="${DetalleReforma.findByReforma(reforma)}"/>
                <g:if test="${detRef}">
                $("#proyecto").val("${detRef?.asignacionOrigen?.marcoLogico?.proyectoId}").change();
                setTimeout(function () {
                    $("#comp").val("${detRef?.asignacionOrigen?.marcoLogico?.marcoLogicoId}").change();
                    setTimeout(function () {
                        $("#actividad").val("${detRef?.asignacionOrigen?.marcoLogicoId}").change();
                        setTimeout(function () {
                            $("#asignacion").val("${detRef?.asignacionOrigenId}").change();
                        }, 500);
                    }, 500);
                }, 500);
                </g:if>
