...
<head>
    ...
    <title>Estados de Garantía</title>
...
<div class="btn-toolbar toolbar">
    ...
    <div class="btn-group">
        <a href="#" class="btn btn-primary btnCrear">
            <i class="fa fa-file"></i> Nuevo Estado de Garantía
        </a>
...
<div style="margin-top: 30px; min-height: 400px" class="vertical-container">
    <p class="css-vertical-text">Estados de Garantía</p>
...
<script type="text/javascript">
    ...
    function cargarTablaTipoGarantia (){
        $.ajax({
            type: 'POST',
            url: '${createLink(controller: 'estadoGarantia', action: 'tablaEstadoGarantia')}',
            data:{
            },
            success: function (msg) {
                $("#tablaEstadoGarantia").html(msg)
            }
        });
    }
    ...
    function submitFormTipoGarantia() {
        var $form = $("#frmSaveEstadoGarantia");
        ...
        var borrarTipoGarantia = {
            ...
            action           : function () {
                bootbox.confirm({
                    ...
                    message: "Está seguro de borrar este Estado de Garantía? Esta acción no puede deshacerse.",
                    buttons: {
                        cancel: {
                    ...
                    callback: function (result) {
                        if(result){
                            ...
                            $.ajax({
                                type: 'POST',
                                url: '${createLink(controller: 'estadoGarantia', action: 'borrarEstadoGarantia_ajax')}',
                                data:{
                                    id: id
                                },
                                success: function (msg) {
                                    dialog.modal('hide');
                                    if(msg == 'ok'){
                                        log("Estado de garantía borrada correctamente","success");
                                        setTimeout(function () {
                                            cargarTablaTipoGarantia();
                                        }, 1000);
                                    }else{
                                        log("Error al borrar el Estado de garantía", "error")
                                    }
                                }
                            });
                        }
