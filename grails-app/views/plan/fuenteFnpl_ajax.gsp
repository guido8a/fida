...
<div class="alert alert-success">
    <form class="form-inline" id="frmFuentePlan">
        <div class="form-group">
            ...
            <g:select name="fuente" from="${fuentes}" optionKey="id" optionValue="fuente"
                      class="form-control input-sm" style="width:250px;"/>
        </div>
        <div class="form-group">
            <label for="montoFin">Disponible</label>
            <div class="input-group input-group-sm" id="divDisponible">
...
<script type="text/javascript">
    ...
    $("#fuente").change(function () {
       var f = $("#fuente option:selected").val();
       cargarDisponible(f)
    });

    function cargarDisponible(fuente) {
        $.ajax({
           type: 'POST',
           url:'${createLink(controller: 'plan', action: 'disponible_ajax')}',
            data:{
               id:fuente
            },
            success: function (msg) {
                $("#divDisponible").html(msg)
            }
        });
    }
    ...
    $(function () {
        ...
        $("#btnAddFin").click(function () {
            if ($frm.valid()) {
                ...
                if (existe) {
                    bootbox.alert('<i class="fa fa-exclamation-triangle text-danger fa-3x"></i> ' + '<strong style="font-size: 14px">' + "Ingrese una fuente no seleccionada aún" + '</strong>');
                } else {
                    if(monto == 0){
                        bootbox.alert('<i class="fa fa-exclamation-triangle text-danger fa-3x"></i> ' + '<strong style="font-size: 14px">' + "Ingrese un valor diferente de cero" + '</strong>');
                    }else{
                        $.ajax({
                            type    : "POST",
